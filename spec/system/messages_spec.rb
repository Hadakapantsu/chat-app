...
RSpec.describe "メッセージ投稿機能", type: :system do
  ...
  context '投稿に失敗したとき' do
    it '送る値が空の為、メッセージの送信に失敗すること' do
      ...
      # DBに保存されていないことを期待する
      expect{
        find('input[name="commit]').click #name="commitどっからきたん
      }.not_to change { Message.count }   #”’。{[]の違いがいなに
      
      # 元のページに戻ってくることを期待する
      expect(current_path).to eq room_messages_path(@room_user.room)   #(@room_user.room)はどっから出てきたん
